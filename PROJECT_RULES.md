...
1. 제품의 공식 명칭·표시 명칭은 `한양`이다. 패키지·모듈 등 기술 식별자(저장소 이름, Swift 모듈 `HanAI`, Kotlin 패키지 `com.hanai.core`)는 `HanAI`를 유지한다.
2. 대표님이 말씀하시는 `한양`은 항상 이 프로젝트를 뜻한다. 대화와 사람 대상 공식 제품·표시 명칭은 `한양`, 코드·패키지·빌드 식별자 등 기술 표기에서는 `HanAI`를 쓴다.
3. `AiShot`은 한양이 제공하는 **골프 기능 이름**으로만 쓴다. 엔진·모델·타입 이름에 `HanClip` 등 특정 앱 브랜드를 넣지 않는다.
...
6. 대표님이 `마무리`라고 하면 HanAI 소스 검토와 관련 검증을 끝내고 이번 변경만 커밋·기본 원격 푸시한다. 대표님이 `릴리즈`라고 하면 HanAI만 별도 프로젝트로 검증·공개 배포하고 NasFinder.com의 독립 제품 정보와 다운로드 경로를 맞춘다. HanClip 반영은 대표님이 HanClip 적용 또는 동기화를 별도로 직접 지시한 경우에만 수행한다.
7. `한양 구현 완료`, `HanAI Git 백업 완료`, `HanAI 공개 배포 완료`, `HanClip 별도 적용 완료`를 구분해 보고한다. 앱별 검증 전에는 한양의 새 기능이 한클립에 적용됐다고 표현하지 않는다.
...
3. 외부 앱 저장소(HanClip 등)는 읽기 전용 참고 자료다. HanAI 학습·마무리·릴리즈 작업에서는 앱 저장소를 수정하지 않는다. 대표님이 HanClip 적용 또는 동기화를 별도로 직접 지시한 경우에만 해당 앱 규칙과 `project-sync` 절차에 따라 수정한다.
