...
한양은 영상·사진에서 "기억할 순간"을 찾는 재사용 가능한 판정 엔진이다. 특정 앱에 종속되지 않으며,
플랫폼 카메라·오디오·관절 추출은 앱 쪽 adapter가 맡고 한양은 **순수 수치 입력 → 판정 출력**만 담당한다.
...
| 이미지 | `ImageSimilaritySelector` | 앱이 추출한 익명 특징 거리와 선명도로 근접 중복 사진의 대표 프레임 선택 |
